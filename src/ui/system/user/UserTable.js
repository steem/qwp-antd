...
let UserTable = React.createClass({
  ...
  handleEditUser() {
    if (this.props.user.selectedUsers.length === 0) {
      message.warning(l('No users are selected, please selected users first'))
      return
    }
    if (this.props.user.selectedUsers.length > 1) {
  ...
  updateModalUsersSelection(selectedUsers) {
    this.props.dispatch({
  ...
  onSelectionChanged(selectedUsers, selectedRows) {
    this.updateModalUsersSelection(selectedUsers)
    this.setState({
  ...
  deleteUsers(id, title) {
    if (!id) {
      ...
      id = this.props.user.selectedUsers.join(',')
    }
  ...
  render () {
    ...
    let tableProps = {
      fetch: uri.ops({m: 'user', ops: 'list', mock: true}),
      ...
      rowSelection: {
        ...
        selectedRowKeys: user.selectedUsers,
      },
    }
